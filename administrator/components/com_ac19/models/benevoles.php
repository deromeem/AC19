...
class Ac19ModelBenevoles extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->select('b.id, b.email, b.latitude, b.longitude, b.published, b.hits, b.modified');
		$query->from('#__ac19_benevoles b');

		// joint la table _users de Joomla
		$query->select('u.email AS courriel')->join('LEFT', '#__ac19_utilisateurs AS u ON u.id=b.email');
		...
		if (!empty($search)) {
			...
			if (stripos($search, 'id:') === 0) {
				...
				$searches	= array();
				$searches[]	= 'courriel LIKE '.$search;
				$searches[]	= 'b.latitude LIKE '.$search;
				$searches[]	= 'b.longitude LIKE '.$search;
		...
		// tri des colonnes
		$orderCol = $this->state->get('list.ordering', 'courriel');
